...
func getUser(sess *session.Session) (*iam.User, error) {
	...
	if resp.User.UserName == nil {
		return nil, errors.New("AWS Account does not have a UserName attribute.  Maybe a root account?")
	}
...
func requestNewSTSToken(awsSession *session.Session, username, mfaToken string, expiry time.Duration, profile string) (*Credentials, error) {
	serial, err := mfaSerialNumber(awsSession, username)
	if err != nil {
		return nil, err
	}
	...
	accessSession, err := newLimitedAccessSession(profile)
	if err != nil {
		return nil, err
	}
	svc := sts.New(accessSession, &aws.Config{Region: aws.String("eu-west-1")})
